...
class ZmopClient {
	...
	public function execute($request) {
		...
		if (isset ($bizRespObject->error_code)) {
			$logger = new \LtLogger;
            if(!file_exists(ZMXY_PATH.'zmop/tmp/logs/')){
                mkdir(ZMXY_PATH.'zmop/tmp/logs/',0777,true);
            }
			$logger->conf["log_file"] =ZMXY_PATH.'zmop/tmp/logs/zmop_biz_err_' . $this->appId . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
					date("Y-m-d H:i:s"),
					$resp
				));
	...
	private function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		$logger->conf["log_file"] = ZMXY_PATH.'zmop/tmp/logs/zmop_comm_err_' . $this->appId . "_" . date("Y-m-d") . ".log";
