...
include_once ZMXY_PATH.'zmop/request/ZhimaCustomerCertificationCertifyRequest.php';
defined('SIGNTYPE') or define('SIGNTYPE','RSA');//签名方式,默认为RSA,可使用RSA2
class ZhimaCustomerCertificationCertify extends Controller
{
//芝麻信用网关地址
     ...
     */
    public function zhimaCertify($pram)
    {
        $client = new \ZmopClient($this->gatewayUrl,$this->appId,$this->charset,$this->privateKeyFile,$this->zmPublicKeyFile);
        $request = new \ZhimaCustomerCertificationCertifyRequest();
        $request->setChannel("apppc");
        $request->setPlatform("zmop");
        $request->setBizNo($pram['biz_no']);// 必要参数
        $request->setReturnUrl($pram['returnUrl']);// 必要参数
